#!/usr/bin/env bash
# nimbus — install lifecycle (reversible, idempotent).
#
#   install.sh (--town | --rig <name>) [--dry-run] [--city <path>] [--no-reload]
#
# Turns the nimbus pack ON at one of two scopes:
#   --town        city-wide: the pack's bin/nimbus + skills become available to
#                 the whole city.
...
#      optional — bin/nimbus also runs under any system python3).
...
#        --town       ->  <city>/pack.toml   [imports.nimbus]
#        --rig <name> ->  <city>/city.toml   [rigs.imports.nimbus] (under the
#                         [[rigs]] entry whose name matches <name>)
#      source is recorded relative to the city root (e.g. "packs/nimbus").
#   3. Trigger re-projection with `gc reload` so the pack's bin/skills surface.
#   4. Verify: `gc lint`, the import is registered, and a `nimbus version` smoke.
#
# Like provider-forge / cockpit, nimbus ships NO prompt fragment and NO overlay
# hook — it is a CLI + skills — so there is no append_fragments edit and no
# projected-settings hook to materialize. The install is: venv + import + reload
# + verify.
#
# This pack lives in the blackrim-nimbus-skills repo under pack/ and is meant to
# be VENDORED into a city as packs/nimbus (see docs/DESIGN.md). Run with
# --city <city-root> if the pack is not already under the target city.
#
...
PACK_NAME="nimbus"      # import binding name
IMPORT_NAME="nimbus"
...
# ---- locate the city -------------------------------------------------------
# Default: the city that physically contains this pack (…/<city>/packs/nimbus).
# When the pack is still in its source repo (blackrim-nimbus-skills/pack), pass --city.
if [ -z "$CITY" ]; then
  CITY="$(cd "$PACK_DIR/../.." && pwd)"
fi
[ -f "$CITY/city.toml" ] || die "no city.toml at city root: $CITY (pass --city <path>; if the pack is in its source repo, vendor it into <city>/packs/nimbus first — see docs/DESIGN.md)"
CITY="$(cd "$CITY" && pwd)"

# Source path recorded in the import: relative to the city root if the pack lives
# under it (a bare relative path with no "./" prefix, e.g. "packs/nimbus"),
# else absolute.
...
# ---- helpers ---------------------------------------------------------------
backup_file() { # back up $1 once per run to <file>.nimbus.bak.<ts>
  local f="$1"
  [ -f "$f" ] || return 0
...
if [ "$NO_RELOAD" -eq 1 ]; then
  ...
  info "[dry-run] gc reload  (surfaces the pack's bin/nimbus + skills)"
else
...
step "4/4  verify"
if [ "$DRY_RUN" -eq 1 ]; then
  info "[dry-run] would verify: gc lint, import registered, nimbus version smoke"
  step "nimbus install — DRY RUN complete (no changes made)"
  exit 0
fi
...
# 4c. nimbus version smoke (best-effort): confirms the bin/nimbus -> python wiring
#     resolves under projection. A non-zero exit means the CLI could not run on
#     THIS machine (no python) — surface as advice, not an install failure (the
#     install itself succeeded).
if [ -x "$PACK_DIR/bin/nimbus" ]; then
  if "$PACK_DIR/bin/nimbus" version >/dev/null 2>&1; then
    info "smoke: bin/nimbus version ok"
  else
    info "smoke: bin/nimbus could not run (no python3?); run 'bin/nimbus version'"
  fi
fi
...
echo
if [ "$fail" -eq 0 ]; then
  step "nimbus install complete ($SCOPE${RIG:+ $RIG})"
  info "Next: 'bin/nimbus info' to see the pack roadmap (the scaffold + deploy"
  info "surface lands in nim-rsa; the golden-path skill in nim-ulq)."
  info "Reverse with: $PACK_DIR/uninstall.sh $REVERSE_FLAGS"
  exit 0
else
  step "nimbus install finished WITH WARNINGS ($SCOPE${RIG:+ $RIG})"
